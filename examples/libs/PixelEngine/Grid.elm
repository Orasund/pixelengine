...
If case of a invalid position, it returns an error. If you do not want to
deal with errors, use `Grid.Wrapped` instead.
...
map fun ((Grid { dict, rows, columns }) as grid) =
    grid
        |> foldl
            (\pos elem g ->
...
foldl fun val ((Grid { dict, rows, columns }) as grid) =
...
foldr fun val ((Grid { dict, rows, columns }) as grid) =
    List.range 0 (columns - 1)
        |> List.foldr
            (\x out ->
                List.range 0 (rows - 1)
